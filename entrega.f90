program entrega_mqn
   ...
   interface 
      ...
      subroutine compute_wave_equation(length, r1, r21, psi_pre, steps, h, psi_next, local_mu1)
         implicit none
      
         integer, intent(in) :: length, steps
         double precision, intent(in), dimension(length) :: psi_pre, r1, r21
         double precision, intent(in) :: h
         double precision, intent(out), dimension(length) :: psi_next, local_mu1
      end subroutine compute_wave_equation

      subroutine calculate_final_properties(length, r1, final_psi, local_mu1)
         implicit none
      
         integer, intent(in) :: length
         double precision, intent(in), dimension(length) :: final_psi, r1, local_mu1
      end subroutine calculate_final_properties
   end interface
   
   common /grosspita/alpha, alpha2, scattering_length, cvar
   common /simulation/step, time_step, atom_numbers, interaction
   common /constants/pi
   double precision :: alpha, alpha2, cvar, pi, step, scattering_length, time_step, interaction, density_param
   double precision, dimension(:), allocatable :: r, r2, prev_psi, next_psi, local_mu
   integer :: width, i, atom_numbers, iterations

   write(*,*) "scattering_length, width, step, atom_numbers, time_step, alpha, iterations"
   read(*,*) scattering_length, width, step, atom_numbers, time_step, alpha, iterations
   ...
   write(*,*) "Precomputing spacial fields"
   do i = 1, width   
      r(i) = dfloat(i-1)*step
      r2(i) = r(i)*r(i)
   end do
...
function initialize_wave_function(step, r, r2, width) result(psi)
   ...
   integer :: i
   common /grosspita/alpha, alpha2, scattering_length, cvar
   double precision :: alpha, alpha2, scattering_length, cvar
...
subroutine calculate_final_properties(length, r, final_psi, local_mu)
   ...
   common /grosspita/alpha, alpha2, scattering_length, cvar
   common /simulation/step, time_step, atom_numbers, interaction
   double precision :: alpha, alpha2, cvar, scattering_length, time_step, interaction, step
